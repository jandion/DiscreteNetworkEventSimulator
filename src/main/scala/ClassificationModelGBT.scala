...
object ClassificationModelGBT{

  def apply(windowSize:Long, leadTime:Long, targetEvent:Int, featureTransformation:Map[Int,Int], model: GradientTreeBoost,queueActor:ActorRef[MessagesPatterns.Command]): Behavior[MessagesPatterns.Command] =
    Behaviors.setup(context => new ClassificationModelGBT(context, windowSize, leadTime, targetEvent, featureTransformation, model,queueActor ))

}

class ClassificationModelGBT(context: ActorContext[MessagesPatterns.Command],
                             windowSize:Long, leadTime:Long, targetEvent:Int,
                             featureTransformation:Map[Int,Int],
                             model: GradientTreeBoost,queueActor:ActorRef[MessagesPatterns.Command])
