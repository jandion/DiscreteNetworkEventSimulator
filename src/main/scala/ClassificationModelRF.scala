...
object ClassificationModelRF{

  def apply(windowSize:Long, leadTime:Long, targetEvent:Int, featureTransformation:Map[Int,Int], model: RandomForest,queueActor:ActorRef[MessagesPatterns.Command]): Behavior[MessagesPatterns.Command] =
    Behaviors.setup(context => new ClassificationModelRF(context, windowSize, leadTime, targetEvent, featureTransformation, model,queueActor))

}

class ClassificationModelRF(context: ActorContext[MessagesPatterns.Command],
                            windowSize:Long, leadTime:Long, targetEvent:Int,
                            featureTransformation:Map[Int,Int],
                            model: RandomForest,queueActor:ActorRef[MessagesPatterns.Command])
