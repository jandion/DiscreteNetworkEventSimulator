...
object ClassificationModelSVM{

  def apply(windowSize:Long, leadTime:Long, targetEvent:Int, featureTransformation:Map[Int,Int], model: SVM[Array[Double]],queueActor:ActorRef[MessagesPatterns.Command]): Behavior[MessagesPatterns.Command] =
    Behaviors.setup(context => new ClassificationModelSVM(context, windowSize, leadTime, targetEvent, featureTransformation, model,queueActor))

}

class ClassificationModelSVM(context: ActorContext[MessagesPatterns.Command],
                             windowSize:Long, leadTime:Long, targetEvent:Int,
                             featureTransformation:Map[Int,Int],
                             model: SVM[Array[Double]],queueActor:ActorRef[MessagesPatterns.Command])
  ...
  def processEvent(e: Seq[Event]): Unit = {
    ...
    if (indexes.nonEmpty) {
      ...
      val prediction: Boolean = model.predict(observationWindow) > 0 //make predicition
